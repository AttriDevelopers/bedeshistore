        ...
        if ($data->status == "completed"){
            ...
            //--- Redirect Section Ends     
                ...
                if ($input['status'] == "completed"){
                    ...
                    $users = User::all();
                    //Get user data(sponser code and referral code) from user id
                    $user = DB::table('users')->select('refer_code','sponsor_code')->where('id', $data['user_id'])->first();
                    $user = (array) $user;
                    $referral_code = $user['refer_code'];
                    $sponser_code = $user['sponsor_code'];

                    $cart = unserialize(bzdecompress(utf8_decode($data->cart)));
                    $finalCommissionTotalAmt = '0';
                    foreach($cart->items as $prod){
                        $totalAmtSingleProduct =  $prod['qty']*$prod['price'];

                        //we not get commission from save card value, so try to find product commission base on product id
                        $product = DB::table('products')->select('commission')->where('id', $prod['item']['id'])->first();
                        $commission = $product->commission;

                        //calculate commission amount using product commission percent
                        $finalCommission = ($totalAmtSingleProduct*$commission)/100;
                        $finalCommission = round($finalCommission,2);
                        $finalCommissionTotalAmt += $finalCommission;
                    }
                    
                    if($sponser_code !=''){
                        for($i=1; $i <= count($users); $i++){
                            //Get user data(sponser code and referral code) from user id
                            $sponsorUser = DB::table('users')->select('current_balance','sponsor_code')->where('refer_code', $sponser_code)->first();
                            $sponsorUser = (array) $sponsorUser;
                            $current_balance = $sponsorUser['current_balance'];
                            
                            $firstLevelCommission = $current_balance + $finalCommissionTotalAmt;
                            User::where('refer_code', $sponser_code)->update([
                            'current_balance' => $firstLevelCommission
                            ]);
                            if($sponsorUser['sponsor_code'] !=''){ 
                                $sponser_code = $sponsorUser['sponsor_code'];
                                $finalCommission = $finalCommissionTotalAmt/3;
                                $finalCommissionTotalAmt = round($finalCommission,2);
                            } else {
                                break;
                            }
                        }
